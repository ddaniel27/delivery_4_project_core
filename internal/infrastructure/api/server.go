...
func (a *App) setupRoutes(g *gin.RouterGroup) {
	handler := a.deps.ProjectsHandler

	recordGroup := g.Group("/project")

	recordGroup.POST("", handler.CreateProject)
	recordGroup.GET("/:id", handler.GetProjectByID)
	recordGroup.GET("/owner/:id", handler.GetProjectsByOwnerID)
}
